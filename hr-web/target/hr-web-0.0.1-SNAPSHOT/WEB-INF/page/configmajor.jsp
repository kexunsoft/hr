...
<div class="notice_main">
			<div class="notice_check">
				...
				<div class="btn-group r_right">
					<a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/Configmajor/configmajorAdd"><span class="glyphicon glyphicon-plus"></span>新增</a>
				</div>
				<div class="clear"></div>
			</div>
			<table id="table" class="table table-condensed" style="margin: 0 auto">
				...
				<c:forEach items="${pageInfo.list}" var="cj">
						<tr>
							<td>${cj.majorKindId}</td>
							<td>${cj.majorKindName}</td>
							<td>${cj.majorId}</td>
							<td>${cj.majorName}</td>
							<td><a href="javascript:del(${cj.majorId})">删除</a></td>
						</tr>
				</c:forEach>
			</table>
...
function del(majorid) {
    layer.confirm('确定要删除这个机构吗?', {icon: 3, title: '提示'}, function (index) {
        ...
        var i;
        $.ajax({
            url: "${pageContext.request.contextPath}/Configmajor/deleteconMajorid",
