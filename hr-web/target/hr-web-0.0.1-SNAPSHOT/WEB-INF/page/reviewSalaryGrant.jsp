...
<div class="notice_main">
    ...
    <table id="table" class="table table-condensed table-bordered" style="margin: 0 auto">
        ...
        <c:forEach items="${salaryGrantDetails}" var="h" varStatus="i">
            <tr name="salaryData">
                <td name="index" data="${h.grdId}">${i.index+1}</td>
                <td>${h.humanId}</td>
                <td>${h.humanName}</td>
                <td salarySum="${h.salaryStandardSum}" id="s${h.humanId}">${h.standardName}</td>
                <td><input type="text" width="100%" id="j${h.humanId}" value="${h.bounsSum}"
                           onkeyup="math(${h.humanId},this)"></td>
                <td><input type="text" width="100%" id="x${h.humanId}" value="${h.saleSum}"
                           onkeyup="math(${h.humanId},this)"></td>
                <td><input type="text" width="100%" id="k${h.humanId}" value="${h.deductSum}"
                           onkeyup="math(${h.humanId},this)"></td>
                <td id="h${h.humanId}">${h.salaryPaidSum}</td>
            </tr>
        </c:forEach>
...
<script>
    ...
    function save() {
        ...
        $.each(tr, function () {
            ...
            //获取项目的id
            var grdId = $($(this).children("td").get(0)).attr("data").length == 0 ? 0 : $($(this).children("td").get(0)).attr("data");
            ...
            json += '{"grdId":' + grdId + ',"humanId":' + humanId + ',"salaryGrantId":"${salaryGrantId}","salaryStandardSum":' + salarySum + ',"bounsSum":' + award + ',"saleSum":' + performance + ',"deductSum":' + deduct + '},';
        ...
        var i;
        $.ajax({
            url: "${pageContext.request.contextPath}/salaryGrant/review",
            data: {"salaryGrantId": "${salaryGrantId}", "salaryGrantDetails": json},
            type: "post",
            beforeSend: function () {
                //						layer.load()
                i = layer.msg('请稍后...', {
            ...
            success: function (data) {
                ...
                if (data.code == 200) {
                    layer.msg(data.message, {
                            ...
                            window.location.href = "${pageContext.request.contextPath}/salaryGrant/reviewBy";
                        }
                    )
                } else {
                    layer.msg(data.message, {
