...
<div class="notice_main">
			<div class="notice_check">
				...
				<div class="btn-group r_right">
					<a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/Configfilethirdkind/configfilethirdkindadd"><span class="glyphicon glyphicon-plus"></span>新增</a>
				</div>
				<div class="clear"></div>
			</div>
			<table id="table" class="table table-condensed" style="margin: 0 auto">
				...
				<c:forEach items="${pageInfo.list}" var="ct">
						<tr>
							<td>${ct.firstKindName}</td>
							<td>${ct.secondKindName}</td>
							<td>${ct.secondKindId}</td>
							<td>${ct.thirdKindName}</td>
							<td>${ct.thirdKindSaleId}</td>
							<td>${ct.thirdKindIsRetail}</td>
							<td><a href="${pageContext.request.contextPath}/Configfilethirdkind/configfilethirdkindmodify?thirdkindid=${ct.thirdKindId}">变更</a></th>
							</th>
							<td><a href="javascript:del(${ct.thirdKindId})">删除</a></td>
						</tr>
					</c:forEach>
			</table>
...
function del(thirdkindid) {
    layer.confirm('确定要删除这个机构吗?', {icon: 3, title: '提示'}, function (index) {

        var i;
        $.ajax({
            url: "${pageContext.request.contextPath}/Configfilethirdkind/deleteconfigfilethirdkind",
            data: {"thirdkindid": thirdkindid},
            type: "get",
            beforeSend: function () {
                i = layer.msg('请稍后...', {
                    icon: 16,
