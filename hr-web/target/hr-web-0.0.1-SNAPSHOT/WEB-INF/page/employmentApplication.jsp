...
<script>
    function del() {
        ...
        layer.confirm('此操作将释放简历,确定要继续操作吗?', {icon: 3, title: '提示'}, function (index) {
            ...
            var i;
            $.ajax({
                url: "/hr/engageInterview/applicationResult?state=0&resumeId=${interview.resumeId}&einId=${interview.einId}",
                type: "get",
                beforeSend: function () {
                    i = layer.msg('请稍后...', {
                        icon: 16,
                ...
                success: function (data) {
                    ...
                    if (data.code == 200) {
                        layer.msg(data.message, {
                                ...
                                window.location.href = "/hr/engageInterview/employmentApplicationList";
                            }
                        )
                    } else {
                        layer.msg(data.message, {
    ...
    function save() {
        var i;
        $.ajax({
            url: "/hr/engageInterview/applicationResult?state=1&resumeId=${interview.resumeId}&einId=${interview.einId}",
            type: "get",
            beforeSend: function () {
                i = layer.msg('请稍后...', {
                    icon: 16,
