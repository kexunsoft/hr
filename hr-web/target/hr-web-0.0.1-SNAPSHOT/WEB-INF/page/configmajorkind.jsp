		...
		<div class="notice_main">
			<div class="notice_check">
				...
				<div class="btn-group r_right">
					<a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/Configmajorkind/configmajorkindAdd"><span class="glyphicon glyphicon-plus"></span>新增</a>
				</div>
				<div class="clear"></div>
			</div>
			<table id="table" class="table table-condensed" style="margin: 0 auto">
				...
				<c:forEach items="${pageInfo.list}" var="ck">
						<tr>
							<td>${ck.majorkindid}</td>
							<td>${ck.majorKindName}</td>
							<td><a href="javascript:del(${ck.majorkindid})">删除</a></td>
						</tr>
				</c:forEach>
			</table>
...
function del(majorkindid) {
    layer.confirm('确定要删除这个机构吗?', {icon: 3, title: '提示'}, function (index) {

        var i;
        $.ajax({
            url: "${pageContext.request.contextPath}/Configmajorkind/deleteconfigmajorkind",
            data: {"majorkindid": majorkindid},
            type: "get",
            beforeSend: function () {
                i = layer.msg('请稍后...', {
                    icon: 16,
            ...
            success: function (data) {
                ...
                if (data.code == 200) {
                    layer.msg(data.message, {
                            ...
                            window.location.href = "${pageContext.request.contextPath}/Configmajorkind/configmajorkindQueryAll";
                        }
                    )
                } else {
                    layer.msg(data.message, {
