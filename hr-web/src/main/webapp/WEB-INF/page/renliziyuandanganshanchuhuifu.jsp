...
<script type="text/javascript">
	    function huifu(obj) {
			var i;
			$.ajax({
						url : '${pageContext.request.contextPath}/HumanFile/huiguByKeyHumanfileBy1?humanid='+obj,
						type : 'get',
						beforeSend : function() {
							i = layer.msg('请稍后....', {
								icon : 16,
							...
							if (data.code == 200) {
								layer
										.msg(
												...
												function() {
													window.location.href='${pageContext.request.contextPath}/HumanFile/selectBydelete';
												})
							} else {
	    ...
	    function del(obj) {
	    	var flag=confirm("确定要删除这条信息嘛?");
	    	if(flag){
	    		var i;
				$.ajax({
						url : '${pageContext.request.contextPath}/HumanFile/deleteByKeyHumanfile?humanid='+obj,
						type : 'get',
						beforeSend : function() {
							i = layer.msg('请稍后....', {
								icon : 16,
		...
		<table id="table"
			...
			<c:forEach items="${pageinfo.list}" var="fth">
				<tr>
					...
					<td><a href="javascript:huifu(${fth.humanId })">恢复</a></td>
					<td><a href="javascript:del(${fth.humanId })">永久删除</a></td>
				</tr>
			</c:forEach>
		</table>
